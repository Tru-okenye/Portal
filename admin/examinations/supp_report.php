...
$suppAttendanceRecords = [];
if ($searchCategory && $searchCourse && $searchYear && $searchIntake && $searchSemester && $searchUnit) {
    $sql = "SELECT * FROM supp_attendance WHERE course_name = ? AND year = ? AND intake = ? AND semester = ? AND unit_code = ?";
...
<head>
    ...
    <title>View Supplementary Attendance</title>
    <style>
    form h2, h3 {
        color: #E39825;
    }
...
<body>
    ...
    <form method="POST" action="">
        <h2>View Supplementary Attendance</h2>
    ...
    <?php if ($searchCategory && $searchCourse && $searchYear && $searchIntake && $searchSemester && $searchUnit): ?>
        <?php if (!empty($suppAttendanceRecords)): ?>
            <table border="1">
                <thead>
                    <tr>
                ...
                <tbody>
                    <?php foreach ($suppAttendanceRecords as $record): ?>
                        <tr>
                            ...
                            <td><?php echo htmlspecialchars($record['intake']); ?></td>
